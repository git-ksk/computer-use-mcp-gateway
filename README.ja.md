...
> **ランタイム状況:** V2 Hub + Agent が推奨される開発・実行経路です。`v0.8.0` Backend Portability & Recovery Safety が現在の released baseline です。V1 は回帰試験・参照用途および既存の本番運用向けに `v1_gateway` として引き続き利用できます。
