...
CI release-candidate archives are a separate distribution-evidence layer, not an installer. `scripts/v2_release_candidate.py` builds them only from an explicit prebuilt-binary directory and a fixed platform allowlist, records per-file SHA-256/size plus exact source identity, and requires fresh-extraction verification before smoke. The candidates intentionally do not copy repository trees, secrets, state, configuration, Handoff runtime data, or recovery-key material. macOS Secure Enclave recovery binaries remain on the existing reviewed single-Mac build/signing path and are not included in the generic candidate bundle until that signing/notarization boundary is separately admitted. See [`../docs/VERSIONING.md`](../docs/VERSIONING.md#release-candidate-artifacts).
