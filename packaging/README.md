...
Release-candidate artifacts are a distribution-integrity layer, not execution authority. Linux/Windows remain reviewed native candidate evidence. The macOS single-Mac candidate additionally implements the #237 source-free install/upgrade profile: exact CUMG/Handoff commits are bound in closed manifests, the local recovery binaries and a self-contained Handoff runtime are included, and activation still requires the operator's existing stable Team-ID code-signing boundary. Secrets/state/private endpoints are never bundled. See [`../docs/v2/V2_RELEASE_ARTIFACTS.md`](../docs/v2/V2_RELEASE_ARTIFACTS.md) and [`../docs/VERSIONING.md`](../docs/VERSIONING.md#release-candidate-artifacts).
...
For one trusted development Mac running Hub + external signer + Agent locally, the normal install/upgrade path is the verified artifact flow in [`../docs/v2/V2_RELEASE_ARTIFACTS.md`](../docs/v2/V2_RELEASE_ARTIFACTS.md), with runtime operations defined by [`../docs/v2/V2_SINGLE_MAC_PRODUCTION.md`](../docs/v2/V2_SINGLE_MAC_PRODUCTION.md). The LaunchAgent templates remain under [`launchd/single-mac/`](launchd/single-mac/). The profile is loopback-only, keeps grant signing external to the Hub, and pairs runtime upgrades with `v2_doctor` verification. `scripts/v2-single-mac-upgrade.sh` is for an already-installed profile; it is not a secret/enrollment bootstrapper.
For an artifact-backed single-Mac cutover, invoke the bundled wrapper as `python3 install/v2_launchd_maintenance_job.py run-upgrade --artifact-bundle <bundle>`; the source-tree invocation remains maintainer-only; do not use `launchctl submit` as an ad-hoc one-shot mechanism. The wrapper explicitly uses `RunAtLoad=true` / `KeepAlive=false`, refuses stale maintenance jobs, never retries a non-zero upgrade, and cleans its temporary launchd job/plist before returning.
