...
use computer_use_mcp_gateway::{
    ...
    v2_online_recovery::{RECOVERY_WEBAUTHN_VERIFIER_FILENAME, load_challenge},
};
...
#[ignore = "trusted physical Windows/Cua/Windows Hello online recovery acceptance; requires explicit ACK, pre-provisioned WebAuthn verifier, and external v2_recover resolve-windows-hello"]
async fn physical_windows_hello_online_recovery_never_replays_ambiguous_cua_operation() -> Result<()>
{
    ...
    let recovery_verifier_source = PathBuf::from(
        std::env::var("CUMG_V2_ONLINE_RECOVERY_WEBAUTHN_VERIFIER_FILE")
            .context("CUMG_V2_ONLINE_RECOVERY_WEBAUTHN_VERIFIER_FILE is required")?,
    );
    let recovery_verifier = std::fs::read(&recovery_verifier_source)
        .context("failed to read provisioned WebAuthn recovery verifier")?;
    if recovery_verifier.is_empty() || recovery_verifier.len() > 4096 {
        return Err(anyhow!(
            "WebAuthn recovery verifier must be bounded and nonempty"
        ));
    }
    ...
    let recovery_verifier_path = hub_state.join(RECOVERY_WEBAUTHN_VERIFIER_FILENAME);
    std::fs::write(&recovery_verifier_path, recovery_verifier)?;
    ...
    // External `v2_recover resolve-windows-hello` performs the real Windows Hello
