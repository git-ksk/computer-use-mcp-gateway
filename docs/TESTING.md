...
Issue #284 additionally composes the Hub runtime with the real PostgreSQL provider as a local preflight: revision A prepares a not-yet-dispatched effect, revision B acquires the next durable writer epoch against the same PostgreSQL row, and the still-live revision A must fail its pre-enqueue persistence with `StaleWriter`/`RevisionConflict`, fence itself, and enqueue no effect. This strengthens replacement evidence before paid/external hosted acceptance, but it does not satisfy the final Cloud Run revision-overlap, partition, shutdown, backup/restore, or physical-Handoff rows.
