...
Issue #353 adds hosted secret/key rotation composition acceptance. `v2_hosted_secret_rotation` proves that revision overlap has exactly one current durable writer epoch, continuity-proven device-key replacement invalidates the old session while preserving `Indeterminate`/quarantine/no-replay state, Hub and grant-signing authorities rotate independently with a bounded verifier overlap, Handoff viewer/transport generations cannot mutate Agent/intervention authority, and OAuth introspection client-secret values remain redacted from debug output. The provider-neutral automated core is not a Cloud Run support claim; real revision A/B + managed-secret + log/OTLP acceptance remains a required input to #284. See [`v2/acceptance/V2_HOSTED_SECRET_ROTATION_ACCEPTANCE.md`](v2/acceptance/V2_HOSTED_SECRET_ROTATION_ACCEPTANCE.md).
