...
Issue #255 extends guided-recovery tests without widening authoritative reconciliation: plan schema v2 keeps `supported_decisions` unchanged, labels the separate Human historical assertion path as interactive-only with automatic selection forbidden, proves observational diagnostics cannot create a decision, and requires exact challenge/incident revalidation while the authoritative set remains empty immediately before signing. The existing recovery protocol still supplies user presence, durable acknowledgement, and no-replay enforcement.
