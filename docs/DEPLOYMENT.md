...
Northbound authentication and CUMG authorization are deliberately separate. The packaged Hub has three explicit, mutually exclusive authentication adapters: RFC 7662 OAuth token introspection, provider-neutral signed OIDC/JWT verification for multi-principal deployments, and a reviewed trusted-proxy/tunnel fixed-principal adapter for intentionally single-principal deployments. Every successful adapter reduces identity to the same `AuthenticatedClientPrincipal { issuer, subject }`; after that point only `DeviceCapabilityAuthorizer` decides `principal -> device -> exact capability`. RFC 7662, JWT/JWKS, and the local proxy are adapters rather than requirements on the CUMG execution-safety core.

OIDC/JWT mode does not require a CUMG user database merely to identify the caller. Configure the exact issuer with `CUMG_V2_OAUTH_AUTHORIZATION_SERVER`, the exact token audience with `CUMG_V2_OIDC_AUDIENCE`, an operator-pinned HTTPS `CUMG_V2_OIDC_JWKS_URI`, an explicit asymmetric `CUMG_V2_OIDC_ALLOWED_ALGORITHMS` list, and the ordinary `CUMG_V2_OAUTH_REQUIRED_SCOPES`. The verifier requires signed `iss`, `aud`, `sub`, and `exp`, validates `nbf` when present, requires `kid`, rejects symmetric/unknown algorithms, enforces JWK `use`/`key_ops`/`alg` compatibility when present, follows no JWKS redirects, bounds JWKS size/key count/cache lifetime, and rate-bounds unknown-`kid` refresh. `CUMG_V2_OIDC_CLOCK_SKEW_SECS`, `CUMG_V2_OIDC_JWKS_CACHE_SECS`, `CUMG_V2_OIDC_UNKNOWN_KID_REFRESH_SECS`, and `CUMG_V2_OIDC_HTTP_TIMEOUT_SECS` tune only those bounded validation behaviors. A stale/failed required refresh fails closed; bearer tokens and complete claims are not logged or sent southbound.

Authorization data remains separate. The current default authorization store is the integrity-protected `CUMG_V2_NORTHBOUND_POLICY_FILE`, loaded into an in-process exact-tuple policy. A future database or external policy engine may sit behind `DeviceCapabilityAuthorizer` without changing the execution-safety state machine.

For an additional narrow-only semantic ceiling, optionally set `CUMG_V2_SEMANTIC_CONSTRAINT_POLICY_FILE` to a private JSON policy described in [`v2/V2_SEMANTIC_AUTHORIZATION.md`](v2/V2_SEMANTIC_AUTHORIZATION.md). The initial typed rules bound finalized `TypeText` UTF-8 bytes and allowlist normalized requested origins for `BrowserNavigate`. The file is loaded once at Hub startup, unknown/malformed fields fail closed, and the exact canonical `(revision, SHA-256 digest)` snapshot is immutable for that Hub runtime. Semantic allow never grants a capability: exact principal/device/`DeviceCapability` authorization is repeated at the finalized-command seam. A stale admitted snapshot cancels before provider dispatch without creating `Indeterminate`; raw text/URL/policy content is not persisted in semantic decision evidence.

For a trusted authenticated proxy/tunnel, constrain the Hub listener to loopback. An explicitly single-principal deployment sets `CUMG_V2_TRUSTED_PROXY_ISSUER`, `CUMG_V2_TRUSTED_PROXY_SUBJECT`, and the required `CUMG_V2_TRUSTED_PROXY_SECRET_FILE`; these are mutually exclusive with both OAuth introspection and OIDC/JWT settings. Generate a random header-safe secret of at least 32 bytes (for example 32 random bytes encoded as hex), store it as a private regular file, and provision the same value independently to the reviewed local proxy. The proxy **must overwrite**, not append or pass through, `X-CUMG-Trusted-Proxy-Token` on every request it forwards to the loopback Hub. The Hub validates and strips that token before global request admission, so an unrelated local process cannot consume the normal northbound concurrency/rate budget merely by reaching `127.0.0.1`.
...
Each unresolved entry identifies the exact `blocking_operation_id`, stable device/generation, capability/semantic operation class, conservative `read_only` or `effectful` class, bounded workflow/client correlation labels when supplied, fingerprint presence, bounded text-input evidence shape when present, dispatch-binding presence (never its value), target/effect/verification classes, durable dispatch marker/timestamps, indeterminate timestamp/reason, available evidence class, `evidence_status`, explicit `reconciliation_status` (`auto_reconciling`, `operator_required`, or `unrecoverable_evidence_gap`), `manual_audit_required`, and a bounded recovery disposition. It deliberately omits authenticated owner issuer/subject, the request fingerprint/key value, and every raw command, argv, cwd, environment value, typed text, URL, clipboard value, screenshot, backend identifier, result payload, credential, or secret. Correlation labels are audit-only and do not authenticate the caller or authorize recovery. The state-directory filesystem permissions remain the authorization boundary for this operation-specific local recovery surface. Northbound `cumg_status` exposes only the separate privacy-bounded unified runtime summary; it does not expose these operation-specific fields, owner identity, evidence payloads, or recovery authority.

A northbound tool request rejected by an existing quarantine returns `code=device_indeterminate`, `blocking_operation_id=op_...`, `retry_safe=false`, `execution_may_have_occurred=true`, `blind_replay_safe=false`, `next_action=get_operation_then_reconcile`, and `follow_up_effectful_operation=new_operation_id_required`. `blocking_operation_id` always names the **earlier ambiguous operation that is already quarantining the device**, not a newly generated ID for the rejected request. These guidance fields are derived from authoritative operation state, never shell-text heuristics. Do not replay that operation. Use `get_operation` first when the exact ID is available, then reconcile independently or through the reviewed recovery flow before creating any fresh effectful operation. `confirmed_not_executed` is valid only with independent evidence that no side effect occurred; `confirmed_completed` is valid only with independent evidence that the intended side effect completed; otherwise leave quarantine intact.
...
Execution-safety schema v11 separates permanent replay denial from bounded detailed retirement history. Current checkpoints store retired IDs in a lossless compact tombstone index capped at 4096 entries; detailed `RetirementRecord` history and corresponding full retired operation detail remain bounded at 64 and rotate oldest-first. Pre-v11 checkpoints migrate their exact legacy retired-ID set on the next current-schema publication. Once any detailed record has rotated, rollback to v10 is rejected because v10 cannot preserve the larger permanent deny set. At tombstone-capacity exhaustion, retirement is refused and quarantine remains authoritative; do not raise the limit as a first response—investigate recurrent indeterminate outcomes. `inspect-reconciliation-history` exposes bounded capacity state, and OTLP emits capacity used/limit plus warning/critical/exhausted signals without operation IDs as metric labels.
...
For the Windows Hello candidate provider (#227), install the public `recovery-webauthn-verifier.json` generated by `v2_recover init-windows-hello` as `<HUB_STATE_DIR>/recovery-webauthn-verifier.json`. Do not install both the macOS direct-P256 verifier and the WebAuthn verifier: conflicting recovery verifiers fail startup closed. Physical Windows Hello interactive acceptance was recorded on 2026-09-19 under #227. Deploy the Windows provider only with the reviewed verifier provisioning and platform-UV requirements described here; offline `v2_maint` remains the break-glass fallback.
