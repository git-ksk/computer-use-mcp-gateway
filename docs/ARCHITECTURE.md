...
Execution-safety schema v10 adds a distinct `current_state_accepted` retirement disposition for reviewed low-impact GUI ambiguity. It reuses the v5 permanent replay tombstone without settling history: the operation remains `Indeterminate`, no terminal receipt is created, and the exact old operation ID remains non-replayable. The online path requires the separately provisioned local-user recovery key, a fresh Hub-signed challenge, exact quarantine fingerprint, exact current authenticated/durable generation, and a strictly newer generation than the original dispatch. Only `Scroll` and `MovePointer` are eligible. The durable audit records `outcome=unknown`, `disposition=current_state_accepted`, `authority=local_user_presence`, bounded evidence metadata, and `replayed=false`. A checkpoint containing this v10 distinction cannot downgrade to v9.
