...
Execution-safety schema v12 durably binds privacy-bounded semantic-authorization admission evidence (snapshot revision/digest plus constraint kind/rule ID). Schema v13 adds explicit mutation-resume barriers/records after acknowledged-unknown PointerClick current-state acceptance and fails closed on new mutations until fresh-generation Human re-arming. Schema v14 adds bounded private recovery-target identity for newly admitted application launch/termination ambiguity. Schema v15 adds Hub-authoritative pre-enqueue non-delivery settlement only when the exact first outbound attempt is proven to fail during encode-before-enqueue or when the closed bounded mpsc returns the unsent frame. Transport/session loss after successful enqueue remains `Indeterminate` / quarantined, and the v15 proof never authorizes replay of the old operation. `inspect-quarantine` may include bounded auto-resolution history, while `audit-reconciliation` identifies this Hub protocol authority separately from Agent evidence.
...
For a future hosted Hub, this Agent-owned rule remains unchanged. Hosted operator/session routing may be public and replaceable, but it is not a second mutation-authority FSM: CUMG authoritative operation/quarantine state stays Hub-owned, canonical Handoff mutation authority stays Agent-owned, and any hosted Handoff routing state is non-authoritative session metadata. The independent Agent generation, Handoff epoch, Desktop/application session, Human viewer generation, transport generation, and hosted writer epoch are defined in [`v2/V2_HOSTED_HANDOFF_TOPOLOGY.md`](v2/V2_HOSTED_HANDOFF_TOPOLOGY.md).
