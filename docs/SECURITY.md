...
When first-class Handoff coordination is enabled, `CUMG_V2_HANDOFF_CONTROL_SOCKET` is a separate local operator plane, not a northbound MCP tool. Keep its parent directory private and non-symlinked; the Hub creates the socket as `0600` but only relays signed/session-fenced control to the Agent-owned canonical Handoff runtime. The local caller cannot submit principal/device/Window authority: CUMG supplies the exact target from a still-valid interaction context and current Agent session fence, and the Agent re-validates device/generation/revision plus the exact command surface immediately before Cua. `begin` is also fenced by CUMG execution state: known unsettled device work is rejected as `handoff_agent_not_idle`, an unresolved CUMG desktop quarantine is rejected as `handoff_device_quarantined`, and the Agent retains its independent final idle check for races. The takeover locator returned by explicit operator control is capability material and must not be copied into generic logs or public reports. Agent-local Handoff runtime failure remains fail-closed and is never auto-restarted to restore authority. Expired Handoff recovery abandonment is an explicit local-operator action only: it requires an exact recovery epoch and clears only the signed Handoff recovery checkpoint after durable deletion succeeds. It never resolves CUMG quarantine, manufactures execution evidence, marks the prior semantic action successful, or authorizes replay.
...
Execution-safety schema v5 adds a separate **unknown-outcome retirement** path for an older `Indeterminate` operation whose terminal outcome can no longer be truthfully reconstructed. Retirement is not settlement: the operation remains historically `Indeterminate`, receives no terminal receipt, and is never rewritten as completed/cancelled/not-executed. The reviewed v1 retirement policy is intentionally narrow (`Scroll` and `MovePointer` only), must be explicitly named by offline maintenance, requires the durable device registry to show a strictly newer generation than the original dispatch, requires `operator_required` or `unrecoverable_evidence_gap`, and is available only through the exclusive offline local-maintenance authority. Under schema v5-v10, durable retirement history is capped at 64 entries and exhaustion fails closed. Schema v11 supersedes only that storage coupling while preserving every retirement/no-replay invariant. The exact old operation ID remains a permanent replay tombstone; clearing quarantine cannot dispatch, resume, re-sign, or reconstruct it. Every retirement persists `outcome=unknown`, capability/policy, original and authorizing generations, prior reconciliation state, bounded reason metadata, local maintenance authority, timestamp, and `replayed=false`. High-impact capabilities fail closed. Schema-v1/v2/v3/v4 checkpoints remain readable within their representational limits, but a checkpoint containing v5 retirement state cannot be downgraded to a schema that would erase that distinction.
...
Execution-safety schema v7 adds a `type_text` operation-evidence envelope without retaining plaintext input. Shape metadata is bounded and capability-specific; element/backend refs and coordinates are deliberately omitted while already-authorized process/window identity may be retained. Optional payload correlation uses a deployment-private, domain-separated HMAC-SHA256 rather than a plain hash so low-entropy typed values are not exposed as a reusable offline guessing oracle. The key/fingerprint are never emitted by normal inspection or telemetry. Key rotation makes comparison unavailable rather than different. Shape/fingerprint evidence remains non-authoritative and cannot settle an operation, remove quarantine, or make replay safe. Schema-v1 through v6 remain readable within their representational limits, while downgrade that would discard a v7 evidence envelope fails closed.

Execution-safety schema v8 keeps quarantine fail-closed for every mutating or uncertain capability while permitting a separate evidence-only read lane from an explicit closed allowlist. Recovery mode never treats caller intent, generic shell text, or capability class alone as proof of read-only behavior. The original `Indeterminate` operation and replay tombstone remain authoritative. A recovery read carries a durable lane marker for audit correlation, cannot resolve the quarantine, and on restart/transport/cancellation/backend ambiguity fails terminally as `recovery_read_interrupted` rather than creating or replacing quarantine. `v2_doctor` exposes only the closed recovery mode value `normal` or `restricted_read_only`; it does not disclose operation payloads or broaden authority. Downgrade that would erase v8-only recovery-lane evidence fails closed.

Execution-safety schema v9 adds status-only durable recovery for effectful Desktop/Browser operations after a northbound response is lost. The caller-retained operation ID remains the exact replay identity, `get_operation` rechecks the original owner and current exact capability authorization, and wrong-owner/unknown IDs remain indistinguishable. Non-process effectful recovery persists only a payload-free terminal marker; it does not retain raw screenshot/UI/browser/clipboard/text/URL/backend result material and never turns terminal status into permission to retry. A v9 marker cannot be represented by a v8 reader, so downgrade fails closed.

Execution-safety schema v10 adds a local-user-presence `current_state_accepted` disposition without creating a new historical resolution state. A valid P-256 recovery-key authorization is necessary but not sufficient: the Hub also requires the exact signed challenge/quarantine fingerprint, the current authenticated registry generation, a strictly newer generation than the ambiguous dispatch, and the closed `TransientUiInteractionV1` capability policy (`Scroll`/`MovePointer` only). The operation remains `Indeterminate`, no terminal receipt/result is synthesized, the exact old operation ID stays permanently tombstoned, and only quarantine is removed after checkpoint publication succeeds. Persistence failure rolls back to quarantine. Durable audit is bounded and records only the unknown outcome, capability/policy/generations, prior reconciliation state, `disposition=current_state_accepted`, `authority=local_user_presence`, bounded Human evidence metadata, timestamp, and `replayed=false`; screenshots and other raw desktop payloads are not part of the contract. This transition is not exposed northbound. A v10 current-state record cannot be represented by a v9 reader, so downgrade fails closed.

Platform recovery providers remain subordinate to the same schema-v10 authority checks. The shared WebAuthn/CTAP ES256 verifier requires exact CUMG challenge/client-data binding, credential ID, RP ID hash, ES256 signature, and signed UP+UV. The Linux FIDO2 candidate additionally pins an explicit root-owned libfido2 tool directory and explicit authenticator device, requires CTAP2/ES256 and an explicit PIN-UV or built-in-UV mode, never accepts PIN material through CUMG config/env/argv/stdin/logs, and has no software-key or U2F fallback. Physical Linux acceptance remains mandatory before claiming that provider supported.

Execution-safety schema v11 separates permanent replay-deny state from bounded detailed retirement audit. The permanent index is lossless rather than probabilistic: canonical 128-bit CUMG operation IDs use a reversible compact encoding and legacy IDs use an exact UTF-8 fallback. The index is independently capped at 4096 entries; reaching that limit refuses further retirement and keeps quarantine authoritative. Detailed `RetirementRecord` history remains capped at 64 and rotates oldest-first, and the corresponding full retired operation detail is compacted when its audit record rotates out. Missing/corrupt/duplicate/mixed-encoding tombstones fail restore closed. Pre-v11 checkpoints migrate by converting their exact legacy retired-ID set; a v11 checkpoint can downgrade to v10 only while every permanent tombstone still has its detailed retirement record. Metrics/state expose bounded used/limit plus 75% warning, 90% critical, and exhausted status without operation IDs as metric labels.

### V2 payload-safe observability

V2 diagnostic output is a security boundary. Default tracing events and OTel metrics must not contain raw `DeviceCommand`/`DeviceResult` values, process stdout/stderr, shell text/argv/environment values, file paths or contents from operation payloads, OAuth bearer tokens or introspection secrets, exact grants, protocol signatures, or private key material. Error and Debug formatting used by the V2 Hub/Agent/backend/persistence boundary is reduced to stable error codes; unexpected signed protocol messages are represented by their message kind rather than by `Debug`-formatting the object. OAuth debug representations redact the introspection client secret and authenticated principal. Northbound process/shell policy failures may expose only a closed privacy-safe category and a fixed message; requested/allowed paths, command text/arguments, environment keys or values, and raw OS spawn/I/O errors are not part of that contract.
...
For a shared physical Cua backend, supported V1 and V2 control planes use one local single-mutating-authority domain. A durable `v1`/`v2` owner plus monotonically increasing epoch is protected by a private cross-process OS file lock. Effectful Cua calls and V2 Human-Handoff entry points require the matching owner; observe-only calls may remain available. Authority switching is explicit CAS, cannot cross an in-flight mutation, and is additionally blocked by V2 quarantine or non-idle Handoff. Process exit releases only the transient lock and never auto-transfers owner authority. Old unfenced legacy Gateway coexistence is a migration refusal, not an auto-retirement signal.
