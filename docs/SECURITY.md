...
When first-class Handoff coordination is enabled, `CUMG_V2_HANDOFF_CONTROL_SOCKET` is a separate local operator plane, not a northbound MCP tool. Keep its parent directory private and non-symlinked; the Hub creates the socket as `0600`. The local caller cannot submit principal/device/Window authority: CUMG supplies the exact target from a still-valid interaction context and current Agent session fence. The takeover locator returned by local `status`/`begin` is capability material for the operator and must not be copied into generic logs or public reports. A managed Handoff child failure remains fail-closed for the Hub generation and is never auto-restarted to restore authority.
