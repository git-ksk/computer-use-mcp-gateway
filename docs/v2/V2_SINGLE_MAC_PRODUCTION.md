...
JSON schema v1 is the stable machine-readable contract. It reports overall operator status, Agent/control-plane connectivity, backend status, the five #226 readiness lanes, quarantine/replay-safety state and #233 incident-review availability, privacy-bounded Handoff lifecycle, mutation-authority owner/epoch, verified runtime identity, read-only online-recovery key readiness, #234 maintenance status/phase, one stable `primary_reason`, and one supported `next_action`. It intentionally omits takeover locators, intervention/recovery IDs or epochs, device/principal identity, commands/argv/cwd/env, typed text, URLs, screenshots, clipboard data, credentials, grants, fingerprints, and private recovery-key material.

When northbound MCP is enabled for the reviewed single-Mac profile, `cumg_status` exposes this same schema-v1 composition as the preferred agent-facing first diagnostic. The packaged Hub sets only the install/run roots needed by the shared status collector; `v2_status` and `cumg_status` therefore use the same `v2_doctor` observations, Handoff status, maintenance record, runtime pairing, recovery readiness, and `build_operator_status` decision path. `cumg_status` is visible only to a principal that already has at least one exact capability grant for that device, but it does not require a live Agent capability advertisement and remains available when the Agent is offline. The call itself is Hub-local/read-only and does not allocate an effectful operation ID or dispatch to the Agent, so it cannot manufacture a persistent quarantine merely by being queried.
