...
Playwright config / test code は Node.js code として実行されるため、安全な command allowlist ではなく deliberately narrower な execution environment 内の arbitrary code として扱います。#114 の initial boundary は explicitly configured Docker/Podman-compatible local provider と digest-pinned image を必須とします。CUMG は provider の install/start、image pull、VM provision を行わず、host ProcessGroup / Windows Job Object 単体を sandbox と主張しません。

fixed provider invocation は network none、read-only container root、全 Linux capability drop、no-new-privileges、PID/memory/CPU/shm ceiling、pwuser、/tmp と /home/pwuser の bounded tmpfs を使います。host bind は approved workspace を /workspace へ read-only、Agent-private per-run artifact directory を /artifacts へ writable で mount する2系統だけです。process environment は fixed value から構成し、Agent/Gateway の HOME、user identity、SSH agent、credential store、Docker socket、browser profile、arbitrary host path を継承しません。

typed request は bounded relative test path と optional bounded project/grep/workers/lifetime のみを受け付けます。absolute path、parent traversal、option-looking path、NUL、arbitrary environment、raw reporter/output/runtime args、host browser profile、CDP/connect endpoint、Docker socket、SSH agent、credential store は request authority になりません。CLI filter 単体を network sandbox とみなさず、external-origin networking は CUMG より下で enforcement できる provider contract ができるまで defer します。

provider lifecycle evidence は terminal proof の一部です。Docker/Podman client process の exit だけでは container stop を証明しません。explicit stop、natural completion、hard expiry は provider cleanup を実行し、provider ps -a で container absence を証明できた場合だけ terminal success を公開します。provider absence を証明できない場合は PlaywrightProviderOutcomeUnproven として既存 effectful Indeterminate / quarantine / no-auto-replay boundary に入ります。Agent startup は Playwright capability advertise 前に owner-scoped orphan cleanup を行い、recovery failure は provider を fail closed し、active mount/evidence の可能性がある artifact を保持します。

default telemetry に raw test path、grep value、workspace/artifact host path、provider container identifier、pwtest_ ref、output body を記録しません。northbound status は bounded artifact count / total bytes のみを公開し、host artifact path は private のままです。
