...
Agent-native process cancellation は GUI-backend cancellation より強い guarantee を持ちます。Unix process group と Windows Job Object が supervised process tree を terminate し、top-level process completion 時には background descendant も cleanup されます。一方 Cua MCP adapter は cancellation を exact in-flight downstream request ID に propagate しますが、それを desktop side effect が止まった proof とは扱いません。そのため propagated cancellation / timeout は `indeterminate` disposition と device quarantine に map されます。
