...
released v0.6 baseline は `CONTROL_SCHEMA_VERSION = 12` / `CAPABILITY_SCHEMA_VERSION = 8` です。released v0.5 は control schema 10 / capability schema 6、historical #106 v0.6 managed-job development pairing は control schema 11 / capability schema 7 でした。#106/#114 により Cua semantic command set 自体は変更しません。capability schema v5 は fresh authenticated generation 後の signed / payload-free Agent reconciliation-report boundary を追加し、v4/v5 mixed peer は stream を部分解釈せず capability-advertisement handshake で fail closed します。control schema mismatch と capability-advertisement schema mismatch も fail closed します。通常の signed Hub/Agent message は 64 KiB application bound を維持し、bounded image/UI/clipboard/region observation はレビュー済み large-result allowance を使用します。clipboard plain text の上限は 1 MiB です。
