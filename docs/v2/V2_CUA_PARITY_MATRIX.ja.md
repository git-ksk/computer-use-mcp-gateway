...
`CONTROL_SCHEMA_VERSION` は version 7、`CAPABILITY_SCHEMA_VERSION` は引き続き version 4 です。control schema mismatch と capability-advertisement schema mismatch は fail closed します。通常の signed Hub/Agent message は 64 KiB application bound を維持し、bounded image/UI/clipboard/region observation はレビュー済み large-result allowance を使用します。clipboard plain text の上限は 1 MiB です。
