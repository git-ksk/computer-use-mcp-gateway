...
accepted production provider は macOS Secure Enclave と Windows Hello です。Windows Hello は #227 で2026-09-19にphysical interactive-desktop acceptanceをPASSし、Linux は #228 で optional FIDO2/CTAP2 実装候補を持ちます。pendingのLinux platformに弱い software-key fallback は追加せず、provider の明示 prerequisite を満たさない deployment は既存 offline maintenance を使用します。
...
`guide` は orchestration / UX だけを追加し、新しい recovery authority は作りません。最初に Hub 署名 challenge を検証し、既存 #233 incident brief を生成して、`IncidentBrief.cumg.supported_decisions` を変更せず guided-recovery plan schema v3 にコピーします。observational diagnostics は説明材料として表示できますが、supported decision を作成・追加できません。JSON contract では authoritative set と `human_historical_assertion` を別フィールドにし、後者は `automatic_selection_allowed=false` として authoritative set が空の場合の Human-only path だけを説明します。

CUMG が support する decision がある場合、authority-bearing guided path は interactive Human terminal を必須にし、その closed set からの明示選択か cancel だけを受け付けます。`supported_decisions=[]` の場合も CUMG の推奨は keep quarantine のままで、machine-supported outcome を作りません。ただし interactive Human は別レーンとして「この exact operation の完了を直接確認した」「実行されなかったことを直接確認した」「分からない」を明示選択できます。前2つは既存 historical `confirmed_completed` / `confirmed_not_executed` を `audit_assessment=inconclusive` のまま再利用し、`supported_decisions` を変更せず Human observation を authoritative CUMG evidence に昇格させません。「分からない」、EOF、cancel、non-interactive input は quarantine を維持します。`--json` は read-only のままで assertion を選択できません。\n\ninteractive review 中は `guide` が reviewed current generation、signed challenge expiry、残り review window を表示します。macOS/Unix では Human input を bounded interval で待ちながら exact challenge を再確認します。5分の challenge が expire するか、nonce / generation / その他の signed review binding が変わった場合、古い prompt は直ちに無効化します。その stale prompt に対して読み取った入力行も破棄し、`user_presence_authentication=not_started` / `authorization=not_published` / `quarantine=retained` / `old_operation_replayed=false` を表示したうえで fresh signed challenge を待ち、incident review を新しく描画します。fresh challenge では必ず fresh Human selection が必要で、以前の番号選択や unsigned Human assertion を引き継ぎません。fresh challenge 待ちは bounded で、renewal が来なければ quarantine を維持したまま終了し、guide の再実行を求めます。

Human selection 後、CLI は署名前に signed challenge と incident brief を直ちに再読します。authoritative selection では supported-decision set が一致している必要があり、Human historical assertion では authoritative set が引き続き空である必要があります。operation/device/original-generation/current-generation/fingerprint/nonce binding または incident state が変わっていれば fail closed で再reviewを要求します。fresh validation 後だけ既存 Secure Enclave helper が macOS user presence を要求し、deny/cancel/timeout/authentication unavailable では authorization を publish せず quarantine を維持します。old operation の retry/replay authority は追加しません。

`authorization=published` は completion ではありません。guided recovery は必ず #109 semantics で exact Hub-signed `RecoveryResolved` ACK を待ち、request/device/current-generation/operation/decision binding を検証した後だけ `durable_completion=verified` と `old_operation_replayed=false` を表示します。その後、exact quarantine の read-only check と既存 `v2_status` JSON を実行します。terminal success では `recovery_complete=true`、exact quarantine clear、`effectful_execution` が ready か、recovery mode が normal かも明示します。すべて healthy なら `recovery_outcome=verified_healthy`、recovery 自体は durable に完了しているが無関係な Handoff/runtime/mutation-authority/backend/recovery-mode 問題が残る場合は `recovery_outcome=verified_with_unrelated_status_problem` と bounded `v2_status` reason を別に表示し、durable recovery 成功を消しません。
...
Recovery challengeは300秒（5分）で期限切れになります。desktopがquarantineのままなら、通常のauthenticated Agent heartbeatを契機にHubがpending challengeを再確認し、期限切れ後はfresh nonceを持つchallengeを再発行します。承認時間切れだけを理由にHub/Agentを再起動する必要はありません。fresh challenge受信時は以前のlocal authorization handoffを無効化します。interactive guide の Human review も challenge-scoped で、expiry または exact binding rollover が起きれば visible prompt を無効化し、stale selection context を破棄して、fresh incident review と新しい Human choice が完了するまで user-presence authentication を開始しません。
