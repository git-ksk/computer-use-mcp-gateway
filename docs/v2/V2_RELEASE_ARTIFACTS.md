...
A CI artifact is not automatically an official GitHub Release asset. `v0.4.0` has not shipped yet; its verified archives remain release-candidate evidence. A later release may promote a reviewed artifact only through the documented release procedure; tags and published assets are never inferred from CI success.
