...
`scripts/v2-single-mac-upgrade.sh` は導入済み single-Mac profile 用の reviewed upgrade helper です。normal operator path は verified artifact 内の helper を `v2_launchd_maintenance_job.py run-upgrade --artifact-bundle <bundle>` で実行し、CUMG/Handoff source checkout を必要としません。artifact mode は durable maintenance transaction / service drain 前に exact CUMG/Handoff pair と inner runtime payload を検証します。maintainer-only source mode では CUMG checkout が clean `main == origin/main`、review 済み Handoff checkout が clean `main == origin/main` かつ exact `CUMG_V2_EXPECTED_HANDOFF_COMMIT` であることも要求します。両 mode とも live quarantine、Handoff active/recovery/faulted、必須 state/service 不在、Cua/signing input 不足では replacement 前に拒否します。
...
normal reviewed install/upgrade path は [`V2_RELEASE_ARTIFACTS.ja.md`](V2_RELEASE_ARTIFACTS.ja.md) の artifact-backed path です。historical source build は maintainer-only として残し、引き続き bounded にします。`CUMG_V2_CARGO_BUILD_JOBS` は default `2`、許可範囲は `1..8`、`CUMG_V2_MIN_BUILD_FREE_MIB` は default `6144` です。preflight は probe file を作らず free space を読み、floor 未満なら maintenance transaction、service drain、mutation-authority migration、install より前に拒否します。preflight 後の Cargo build で `ENOSPC` / `No space left on device` が発生した場合は `build_storage_exhausted` として記録し、installed runtime と mutation authority を変更する前に終了します。capacity recovery のために Git WIP / untracked file を自動削除することはありません。
...
reviewed single-Mac profile で northbound MCP を有効にすると、`cumg_status` が同じ schema-v1 composition を agent-facing の推奨 first diagnostic として公開します。packaged Hub は shared status collector に必要な install/run root だけを設定し、`v2_status` と `cumg_status` は同じ `v2_doctor` observation、Handoff status、maintenance record、runtime pairing、recovery readiness、`build_operator_status` decision path を使います。`cumg_status` はそのdeviceに少なくとも1つ exact capability grantを既に持つprincipalにだけ表示しますが、live Agent capability advertisementは要求せず、Agent offline時も利用できます。call自体はHub-local/read-onlyで、effectful operation IDを割り当てずAgentへdispatchしないため、status照会だけでpersistent quarantineを作りません。
...
report には operator automation 向けの additive な `readiness` summary を含めます。readiness は第二の authorization model ではなく、既存の authenticated Agent/Hub state、advertised capability、backend diagnostics、mutation authority、durable quarantine state から lane ごとに導出します。lane は `control_plane`、`computer_use_observation`、`filesystem_observation`、`effectful_execution`、`browser_effectful_execution` です。observation lane が `ready` でも effectful authority を意味しません。既存の top-level `overall` は conservative aggregate doctor result のままなので、bounded observation lane が `ready` でも `overall=unsafe` になり得ます。lane availability が必要な caller は `overall` を独自解釈せず `readiness` を確認します。

durable blocking operation が存在する場合、supported effectful lane は `indeterminate_fenced`、`blocking_operation_retry_safe=false`、`operator_action=inspect_reconciliation_status` を返し、独立に確認できた observation lane は `ready` のまま利用できます。transport/backend/state evidence が欠落・不整合なら、該当 lane は healthy と推測せず `unavailable` または `unknown` に fail closed します。`unsupported` は authenticated device がその lane を advertise していないことを意味します。この summary は `confirmed_completed` / `confirmed_not_executed` を推論せず、quarantine clear、retry、replay、dispatch も行いません。
