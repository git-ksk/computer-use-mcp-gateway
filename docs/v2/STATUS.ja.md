...
- **Optional first-class Human Handoff:** CUMGではbounded Window / Terminal-PTY coordinationをaccepted済みです。Windowはupstream `WindowHandoffAdapter`、Terminalはupstream `TerminalHandoffAdapter` を使い、PTY/process containmentとcontent-free verificationはCUMGに残します。managed runtimeはHandoff experimental Terminal moduleへ直接依存せず、production stagingはservice drain前にfirst-class exportをpreflightします。upstream Window #85にはfirst-class same-LAN direct physical rerunが残りますが、Terminal first-class real PTY E2E / physical iPhone Human acceptanceは完了済みです。
...
- **Unknown-outcome retirement:** execution-safety schema v5 は、truthful に reconcile できない legacy ambiguity 向けに offline local-maintenance-only retirement transition を追加します。operation は `Indeterminate` のまま、exact operation ID は permanent non-replayable のまま維持し、`outcome=unknown` / policy / authority / generation fence / `replayed=false` を記録し、quarantine だけを clear します。最初の reviewed policy は `Scroll` と `MovePointer` のみに限定し、strictly newer な durable device generation を要求し、high-impact capability は fail closed します。`inspect-quarantine` は retirement eligibility を示し、reconciliation history は operator reason text を公開せず retired disposition を表示します。maintenance は versioned policy を明示指定し、retirement history は64件に bounded です。
