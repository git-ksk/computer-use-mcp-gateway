...
CI artifact は自動的に official GitHub Release asset にはなりません。既存 `v0.3.0` GitHub Release は source-only のままです。将来の release で artifact を昇格する場合も documented release procedure を通し、CI success だけで tag / published asset を作りません。
