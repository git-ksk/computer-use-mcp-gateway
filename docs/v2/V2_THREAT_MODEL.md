...
The M1 northbound boundary implements the MCP Authorization protected-resource side rather than a new OAuth/OIDC server. `v2_hub` publishes RFC 9728 metadata, requires header bearer presentation, and supports either RFC 7662 introspection or local signed OIDC/JWT verification before reducing the verified identity to `AuthenticatedClientPrincipal`; the trusted-proxy adapter remains a separate fixed-principal option. OIDC/JWT mode pins the configured HTTPS JWKS endpoint, rejects redirects and symmetric/unexpected algorithms, requires exact configured `iss`/`aud` plus `sub`/`exp`, validates `nbf` when present, and bounds JWKS/cache/unknown-`kid` refresh. Required OAuth scopes gate entry to the MCP resource, while the separate local principal -> device -> exact `DeviceCapability` policy remains authoritative for delegated device access. The bearer header is removed before rmcp handler dispatch and no bearer field exists in the typed Hub-to-Agent command/grant path.

The optional #221 semantic-constraint layer is a second, narrow-only Hub admission check after exact capability authorization and after the constrained CUMG command has reached its final normalized dispatchable representation. The initial contracts are an exact UTF-8 byte ceiling for `TypeText` and a normalized requested-origin allowlist for `BrowserNavigate`. A private authorized-command wrapper binds the command that was evaluated; provider adapter tests require the constrained text/URL field to be materialized unchanged. The operator snapshot is immutable as an exact `(revision, digest)` pair for a Hub runtime. Allowed operations persist only revision/digest/kind/rule ID, never the constrained text or URL. A stale snapshot is cancelled before dispatch and does not become `Indeterminate`. Requested-origin policy is explicitly not redirect confinement. See [`V2_SEMANTIC_AUTHORIZATION.md`](V2_SEMANTIC_AUTHORIZATION.md).

Residual risk: authorization-server/introspection or JWKS availability, upstream signing-key compromise, stale-but-still-within-cache keys, and operator misconfiguration of issuer/audience/algorithm ceilings remain deployment trust dependencies. An unknown key never becomes fail-open; once a required JWKS refresh cannot be completed, validation fails closed. Public HTTPS termination and transport-edge rate limiting remain deployment responsibilities outside the loopback northbound listener. In packaged production a compromised `v2_hub` no longer possesses the grant-signing private key and cannot mint a grant while the external signer is absent or rejecting the request. A live signer may still authorize any exact capability intentionally present in its independent signer policy, so OAuth plus key isolation does not make a fully compromised Hub harmless.
...
## Sandboxed Playwright execution

Playwright configuration and test code execute as Node.js code and are treated as arbitrary code inside a deliberately narrower execution environment, not as a safe command allowlist. The initial #114 boundary requires an explicitly configured Docker/Podman-compatible local provider and a digest-pinned image. CUMG does not install/start the provider, pull images, provision VMs, or claim that a host ProcessGroup/Windows Job Object is a sandbox.

The fixed provider invocation denies host-network access with network none, uses a read-only container root, drops all Linux capabilities, enables no-new-privileges, sets PID/memory/CPU/shm ceilings, runs as pwuser, supplies bounded tmpfs for /tmp and /home/pwuser, mounts one approved workspace read-only at /workspace, and mounts one Agent-private per-run artifact directory writable at /artifacts. The process environment is fixed and does not inherit Agent/Gateway HOME, user identity, SSH agent, credential-store, Docker socket, browser profile, or arbitrary host-path state.

The typed request accepts bounded relative test paths plus optional bounded project/grep/workers/lifetime values. Absolute paths, parent traversal, option-looking paths, NUL, arbitrary environment, raw reporter/output/runtime arguments, host browser profiles, CDP/connect endpoints, Docker sockets, SSH agents, and credential stores are not request authority. CLI filtering alone is not treated as a network sandbox; external-origin networking remains deferred until enforceable below CUMG.

Provider lifecycle evidence is part of terminal proof. Docker/Podman client exit alone does not prove the container stopped. Explicit stop, natural completion, and hard expiry perform provider cleanup and require a provider ps -a absence proof before exposing a terminal success state. Failure to prove provider absence becomes PlaywrightProviderOutcomeUnproven and follows the existing effectful Indeterminate, quarantine, and no-auto-replay boundary. Agent startup performs owner-scoped orphan cleanup before Playwright capability advertisement; failed recovery fails the provider closed and preserves artifacts that might still be mounted or evidentiary.

Default telemetry must not record raw test paths, grep values, workspace/artifact host paths, provider container identifiers, pwtest_ refs, or output bodies. Northbound status exposes only bounded artifact count/total bytes; host artifact paths remain private.
...
The Windows provider does not turn Windows account/process identity into recovery authority. It delegates signing only to the native WebAuthn platform authenticator and requires an exact dedicated credential plus User Present and User Verified flags. Task Scheduler, the Agent device key, ordinary process identity, an external security key, or a software private key cannot satisfy this provider. The Hub still verifies the fresh CUMG recovery challenge, quarantine fingerprint, generations, exact credential/public key, RP ID hash, client-data binding, UP+UV flags and ES256 signature before any persistence transition. Cancel/deny or platform failure leaves quarantine unchanged.
