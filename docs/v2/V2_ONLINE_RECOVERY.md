...
The accepted production providers are macOS Secure Enclave and Windows Hello; the Windows physical interactive-desktop acceptance passed on 2026-09-19 under #227. Linux now has an optional FIDO2/CTAP2 implementation candidate in #228. The pending Linux platform gains no software-key substitute, and deployments that do not meet a provider's explicit prerequisites continue to use the existing offline maintenance path.
...
The guided path is orchestration only. It first verifies the Hub-signed challenge, builds the existing #233 incident brief, and copies `IncidentBrief.cumg.supported_decisions` unchanged into guided-recovery plan schema v3. Observational diagnostics can be displayed as explanation but cannot create or widen a supported decision. The JSON contract keeps the authoritative set separate from `human_historical_assertion`; that second field is explicitly `automatic_selection_allowed=false` and exists only to explain a Human-only path when the authoritative set is empty.

When an authoritative decision is available, `guide` requires an interactive Human terminal and accepts only an explicit selection from that closed set (or cancellation). When `supported_decisions=[]`, CUMG still recommends keeping quarantine and does not claim a machine-supported outcome. The interactive Human may separately choose “I directly observed this exact operation complete”, “I directly observed this exact operation did not execute”, or “I do not know”. The first two reuse the existing historical `confirmed_completed` / `confirmed_not_executed` recovery with `audit_assessment=inconclusive`; they do not modify `supported_decisions` or promote observation into authoritative CUMG evidence. “I do not know”, EOF, cancellation, or non-interactive input keeps quarantine. `--json` remains read-only and can never select an assertion.\n\nDuring an interactive review, `guide` displays the reviewed current generation, the signed challenge expiry, and the remaining review window. On macOS/Unix it waits for Human input in bounded intervals while re-checking the exact challenge. If the 5-minute challenge expires or its signed review identity changes because of nonce/generation/other binding rollover, the old prompt is invalidated immediately. Any line read against that stale prompt is discarded, `user_presence_authentication=not_started` / `authorization=not_published` / `quarantine=retained` / `old_operation_replayed=false` are reported, and the CLI waits for a fresh signed challenge before rendering the incident review again. A fresh challenge always requires a fresh Human selection; an earlier numbered choice or unsigned Human assertion is never carried forward. The fresh-challenge wait is bounded; if renewal does not arrive, the command exits with quarantine unchanged and asks the operator to re-run the guide.

After any Human selection, the CLI immediately re-reads the exact signed challenge and incident brief. For an authoritative selection, the exact supported-decision set must still match. For a Human historical assertion, the authoritative set must still be empty. Any operation/device/original-generation/current-generation/fingerprint/nonce binding change or incident-state change fails closed and requires a new review before signing. Only after that fresh validation does the existing Secure Enclave helper request macOS user presence. Denial, cancellation, timeout, or an unavailable authentication facility does not publish authorization and leaves quarantine intact. The guided authorization remains the same reviewed online-recovery protocol; no new recovery authority or replay path is introduced.

`authorization=published` is not completion. Guided recovery always waits for the exact Hub-signed `RecoveryResolved` acknowledgement using #109 semantics. The request/device/current-generation/operation/decision binding must verify, after which the workflow reports `durable_completion=verified` and `old_operation_replayed=false`. It then performs a read-only exact quarantine check and invokes the existing `v2_status` JSON surface. Terminal success also reports `recovery_complete=true`, exact quarantine clearance, whether `effectful_execution` is ready, and whether recovery mode is normal. A healthy status yields `recovery_outcome=verified_healthy`; if recovery completed durably but an unrelated Handoff/runtime/mutation-authority/backend/recovery-mode problem remains, the workflow reports `recovery_outcome=verified_with_unrelated_status_problem` and exposes the bounded `v2_status` reason rather than erasing the durable recovery result.
...
JSON mode never prompts, signs, publishes, clears quarantine, or replays work. It excludes the private challenge fingerprint/nonce and credential/key/raw-command/argv/clipboard/screenshot/principal material. `supported_decisions` remains the authoritative CUMG set; `human_historical_assertion` is separately labelled, requires an interactive Human, and declares automatic selection forbidden. It is therefore suitable for an Agent/UI to explain both paths without allowing that Agent/UI to make either recovery decision.
...
A recovery challenge expires after 300 seconds (5 minutes). While the desktop remains quarantined, normal authenticated Agent heartbeats cause the Hub to re-check the pending challenge and issue a fresh nonce-bound challenge after expiry. An operator therefore does not need to restart the Hub or Agent merely because a local approval window elapsed. Receiving a fresh challenge invalidates the prior local authorization handoff. The interactive guide also treats its Human review as challenge-scoped: expiry or exact binding rollover invalidates the visible prompt, discards the stale selection context, and requires the refreshed incident review plus a new Human choice before any user-presence authentication can start.

### Windows Hello provider (#227, physical acceptance PASS 2026-09-19)

The Windows provider uses the native Win32 WebAuthn broker with a dedicated platform-authenticator ES256 credential. Provisioning requires `WebAuthNIsUserVerifyingPlatformAuthenticatorAvailable`, platform attachment, and required user verification; CUMG exports only `recovery-webauthn-verifier.json` containing the bounded credential ID and X9.63 public verifier. Assertions are restricted to that exact credential, RP ID `cumg-recovery.invalid`, platform attachment, and signed UP+UV flags. There is no external-authenticator, password, software-key, Agent-key, or unattended Task Scheduler fallback.

Use `v2_recover init-windows-hello --verifier-out <FILE>` on the interactive Windows desktop, copy that public verifier to `<HUB_STATE_DIR>/recovery-webauthn-verifier.json`, then use `resolve-windows-hello` or `accept-current-state-windows-hello` with the same local public verifier file. Support must not be claimed until physical Windows Hello UI acceptance proves approve/deny, durable exact-quarantine resolution, restart durability, and no replay. Until then, Windows deployments retain offline `v2_maint` as the supported recovery path.
