...
The runtime supports three explicit authentication adapters: RFC 7662 OAuth introspection, provider-neutral signed OIDC/JWT for multi-principal deployments, and a trusted authenticated-proxy fixed-principal adapter for an intentionally single-principal deployment. OIDC/JWT verifies an operator-configured issuer, exact audience, pinned HTTPS JWKS, asymmetric algorithm allowlist, time claims, and stable subject before emitting the same principal type; provider-specific behavior terminates at that adapter. The trusted-proxy adapter requires a loopback origin, takes principal identity only from operator configuration, and does not trust client identity headers.
