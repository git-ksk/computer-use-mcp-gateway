...
> **Japanese:** [V2_POSITIONING.ja.md](V2_POSITIONING.ja.md)
...
The runtime supports RFC 7662 OAuth introspection and a trusted authenticated-proxy fixed-principal adapter for an intentionally single-principal deployment. OIDC/JWT remains the preferred general signed-token direction for providers such as Firebase/Auth0/Cognito/Entra/Keycloak. The trusted-proxy adapter requires a loopback origin, takes principal identity only from operator configuration, and does not trust client identity headers; multi-principal policy requires a tamper-resistant authenticated identity.
