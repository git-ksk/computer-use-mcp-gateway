...
実行安全性の分類では、読み取り専用であるかどうかはセマンティックコマンド自体が定義します。変更を伴うコマンドのディスパッチ後の汎用バックエンドエラー、応答喪失、または不正・証明不能な完了は、終局的失敗エビデンスとして受け入れられません。それは `BackendOutcomeUnproven` 隔離になります。明示的に読み取り専用のコマンドに対する対応するイベントは、明確なバックエンドエラーのままです。レビュー済みのセマンティック拒否は、型付きの拒否コードを保持します。
