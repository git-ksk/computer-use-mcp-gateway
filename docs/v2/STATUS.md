...
- **Optional first-class Human Handoff:** accepted in CUMG for bounded Window and Terminal/PTY coordination. Window uses upstream `WindowHandoffAdapter`; Terminal uses upstream `TerminalHandoffAdapter` while CUMG retains PTY/process containment and content-free verification. The managed runtime no longer depends directly on Handoff experimental Terminal modules, and production staging preflights the first-class exports before service drain. Upstream Window #85 still tracks the first-class same-LAN direct physical rerun; Terminal first-class real-PTY E2E and physical iPhone Human acceptance are complete.
...
- **Unknown-outcome retirement:** execution-safety schema v5 adds an offline, local-maintenance-only retirement transition for policy-eligible legacy ambiguity that cannot be reconciled truthfully. It leaves the operation `Indeterminate`, keeps the exact operation ID permanently non-replayable, records `outcome=unknown` / policy / authority / generation fence / `replayed=false`, and clears only quarantine. The first reviewed policy admits only `Scroll` and `MovePointer`, must be explicitly pinned by maintenance, requires a strictly newer durable device generation, caps retirement history at 64 records, and rejects policy/capacity mismatch or high-impact capabilities fail closed. `inspect-quarantine` reports retirement eligibility; reconciliation history reports retired dispositions without exposing the operator reason text.
