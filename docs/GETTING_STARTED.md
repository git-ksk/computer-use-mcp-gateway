...
`v0.6.0` is a source-only GitHub pre-release. CI may expose verified **release-candidate** archives for packaging review, but those unsigned/unnotarized artifacts are not official binary Release assets or a broader platform-support claim. See [`VERSIONING.md`](VERSIONING.md#release-candidate-artifacts) for the bounded manifest/checksum/fresh-extraction contract.
