...
> **Note:** This is the canonical English guide. A Japanese translation is available at [GETTING_STARTED.ja.md](GETTING_STARTED.ja.md).
...
The northbound MCP is not a raw Cua proxy. The current typed V2 contract is grouped into Agent-native process/shell execution plus bounded filesystem observation, Desktop semantic capabilities including native same-context element actions, and Browser semantic capabilities plus bounded upload/download transfer when the exact policy and live advertisement permit them. Discovery is filtered by exact authorization and, while the Agent is online, its live `CapabilityAdvertisement`. See [`V2_GUI_SEMANTIC_CAPABILITIES.md`](v2/V2_GUI_SEMANTIC_CAPABILITIES.md) and [`V2_BROWSER_SEMANTIC_CAPABILITIES.md`](v2/V2_BROWSER_SEMANTIC_CAPABILITIES.md).
