...
execution-safety schema v7 は plaintext input を保持せずに `type_text` operation-evidence envelope を追加します。shape metadata は bounded / capability-specific とし、element/backend ref や coordinate は保持せず、すでに authorize 済みの process/window identity だけを必要な範囲で保持できます。optional な payload correlation は plain hash ではなく deployment-private / domain-separated HMAC-SHA256 を使用し、low-entropy な入力値を再利用可能な offline guessing oracle にしません。通常 inspection / telemetry は key/fingerprint を公開せず、key rotation 時の comparison は different ではなく unavailable になります。shape/fingerprint evidence は non-authoritative のままで operation settlement、quarantine clear、replay safety を変更できません。schema v1〜v6 は representational limit 内で引き続き読めますが、v7 evidence envelope を失う downgrade は fail closed します。

execution-safety schema v8 は mutating または side effect が不確実な capability を quarantine 中も fail-closed のまま維持し、explicit closed allowlist に限って独立した evidence-only read lane を許可します。caller intent、generic shell text、CapabilityClass だけを read-only の根拠にはしません。元の `Indeterminate` operation と replay tombstone は authoritative のままです。recovery read は audit correlation 用の durable lane marker を持ち、quarantine を resolve できません。restart / transport / cancellation / backend ambiguity が起きても新規または置換 quarantine を作らず、`recovery_read_interrupted` の terminal Failed として終了します。`v2_doctor` が公開する recovery mode は closed value の `normal` / `restricted_read_only` のみで、payload disclosure や authority 拡張はありません。v8-only recovery-lane evidence を失う downgrade は fail closed します。

execution-safety schema v9 は northbound response loss 後の effectful Desktop/Browser operation に status-only durable recovery を追加します。caller-retained operation ID は exact replay identity のままで、`get_operation` は original owner と current exact capability authorization を再確認し、wrong-owner / unknown ID は indistinguishable のままです。process 以外の effectful recovery が persist するのは payload-free terminal marker だけで、raw screenshot/UI/browser/clipboard/text/URL/backend result material は保持せず、terminal status が retry permission になることもありません。v9 marker は v8 reader で表現できないため downgrade は fail closed します。

execution-safety schema v10 は historical resolution state を追加せず、local-user-presence `current_state_accepted` disposition を追加します。有効な P-256 recovery-key authorization だけでは足りず、Hub は exact signed challenge/quarantine fingerprint、current authenticated registry generation、ambiguous dispatch より strictly newer generation、closed `TransientUiInteractionV1` policy（`Scroll` / `MovePointer` のみ）をすべて要求します。operation は `Indeterminate` のまま terminal receipt/result を synthesize せず、exact old operation ID は permanent tombstone のまま、checkpoint publication 成功後だけ quarantine を外します。persistence failure は quarantine へ rollback します。durable audit は unknown outcome、capability/policy/generation、prior reconciliation state、`disposition=current_state_accepted`、`authority=local_user_presence`、bounded Human evidence metadata、timestamp、`replayed=false` に限定し、screenshot 等の raw desktop payload は contract に含めません。この transition は northbound に公開しません。v10 current-state record は v9 reader で表現できないため downgrade は fail closed します。

platform recovery provider は同じ schema-v10 authority check に従います。shared WebAuthn/CTAP ES256 verifier は exact CUMG challenge/client-data binding、credential ID、RP ID hash、ES256 signature、signed UP+UV を要求します。Linux FIDO2 candidate はさらに explicit な root-owned libfido2 tool directory と authenticator device を pin し、CTAP2/ES256 と明示 PIN-UV または built-in-UV mode を要求します。PINをCUMG config/env/argv/stdin/logへ渡さず、software-key/U2F fallbackも持ちません。Linux provider support を claim する前に physical Linux acceptance が必須です。

execution-safety schema v11 は permanent replay-deny state と bounded detailed retirement audit を分離します。permanent index は probabilistic ではなく lossless です。canonical 128-bit CUMG operation ID は reversible compact encoding、legacy ID は exact UTF-8 fallback を使用します。index は独立して4096件上限で、到達時は追加 retirement を拒否し quarantine を authoritative のまま維持します。詳細 `RetirementRecord` は64件の oldest-first rotation とし、detail が落ちる際は対応する full retired operation detail も compact しますが replay deny は残ります。missing/corrupt/duplicate/mixed-encoding tombstone は restore 時に fail closed します。pre-v11 checkpoint は exact legacy retired-ID set を変換して migrate し、v11 から v10 への downgrade は全 tombstone に詳細 record が残る場合だけ可能です。metric/state は operation ID を label にせず used/limit、75% warning、90% critical、exhausted を公開します。
