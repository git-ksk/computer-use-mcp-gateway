...
- define the separate higher-risk shell-command capability. Structured process execution remains `Dangerous` and intentionally non-sandboxed; the implemented bounded filesystem surface is read-only and does not imply that arbitrary processes are confined to those roots;
