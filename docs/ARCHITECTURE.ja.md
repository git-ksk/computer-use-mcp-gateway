...
execution-safety schema v12 は semantic authorization admission を privacy-bounded な snapshot revision/digest + constraint kind/rule ID としてdurable bindします。schema v13 は acknowledged-unknown PointerClick current-state acceptance 後の explicit mutation-resume barrier/record を追加し、fresh-generation Human re-arm 前のmutationをfail closedにします。schema v14 は新規application launch/termination ambiguity向けにbounded private recovery-target identityを追加します。schema v15 はHubがexact first outbound attemptについてencode-before-enqueue、またはclosed mpscがunsent frameを返したことをauthoritativeに証明できる場合だけ pre-enqueue non-delivery をterminal `Cancelled` / confirmed-not-executedとしてdurable auto-resolveします。successful enqueue後のtransport/session lossは引き続き `Indeterminate` / quarantineとなり、v15 proofはold operation replayを一切authorizeしません。`inspect-quarantine` はbounded auto-resolution historyを併記でき、`audit-reconciliation` はこのHub protocol authorityをAgent evidenceと区別して表示します。
...
v0.8 portability proofでは、同じseamのdeliberately narrowなsecond real implementationとして `MacosMcpAdapter` を追加します。macos-mcp 0.4.0 の `Snapshot` はbounded `ListApplications`、`Move` は `MovePointer`、`Click` は `PointerClick` にmapします。provider自身はCUMG operationをsettleできません。effectful outcomeを失った/証明できない場合は `BackendOutcomeIndeterminate` のままで、Hub fencingとdurable quarantineがauthoritativeです。provider-backed acceptanceは [`V2_SECOND_BACKEND_PORTABILITY_ACCEPTANCE.md`](v2/acceptance/V2_SECOND_BACKEND_PORTABILITY_ACCEPTANCE.md) に記録します。

Direct process/shell execution は Agent 自身が所有し、Cua 経由で terminal window を automation する形で実装してはいけません。structured argv execution は implicit shell parsing を避けますが `Dangerous` のままです。script/interpreter は arbitrary code を実行でき、cwd policy は argv filesystem access を confine しません。そのため M1 の Agent-native process grant は exact device capability に scope します。別の `ReadFile`/`ListDirectory` observation capability は独立した operator-approved file root 配下で path を canonicalize し、returned content を bound し、symlink escape を拒否し、coarse error を返します。この file root は process/shell cwd root とは別設定で、広い execution cwd を許しながら read-only filesystem tree だけ狭くできます。file root 未設定時は cwd から推測せず fail closed します。これら read-only operation は narrower capability surface ですが `ExecuteProcess` 自体を制限しません。free-form shell execution は exact grant と distinct command/result type を持つ separate implemented `Dangerous` capability です。固定 OS shell を intentional に invoke するため、`ExecuteProcess` を widen せず shell parsing risk を受け入れます。explicit filesystem mutation は separate future higher-risk surface のままです。 Windows の shell contract は明示的に `cmd.exe /D /S /C` です。そのため `%NAME%`、`&&`、pipe/redirection、`cmd.exe` の quoting rule を使い、command 全体を追加の quote pair で包みません。`env_clear()` 後の child environment は引き続き allowlist 制で、Windows は cryptographic/runtime initialization と inbox utility に必要な最小 OS locator `SystemRoot` と既存の developer/user locator だけを引き継ぎ、interactive profile 全体は継承しません。captured Windows output は UTF-8 に加えて認識可能な UTF-16 を MCP text へ normalize します。

`env_clear()`後のexecution environmentはraw valueを公開せずdiscoverableにします。`PATH`は存在する場合にAgent service environmentからだけinheritし、login / interactive shell profileはsourceしません。caller-facing `env` mapから`PATH`をoverrideすることはできず、service `PATH`外のexecutableにはreview済みabsolute executable pathを使うのがsupported deterministic routeです。`execute_process` / `shell` tool schemaはraw effective `PATH`を出さず、payload-freeな `x-cumg-execution-environment` metadataでpolicyだけを公開します。macOSではlaunchd service environmentがTerminal/Homebrew設定と大きく異なり得ます。exact service `PATH`はdeployment stateでありprotocol constantではありません。
...
hosted Hub へ進む場合も、この Agent-owned rule は変更しません。hosted operator/session routing は public / replaceable にできますが、2つ目の mutation-authority FSM にはしません。CUMG authoritative operation/quarantine state は Hub-owned、canonical Handoff mutation authority は Agent-owned、hosted Handoff routing state は non-authoritative な session metadata とします。独立した Agent generation、Handoff epoch、Desktop/application session、Human viewer generation、transport generation、hosted writer epoch は [`v2/V2_HOSTED_HANDOFF_TOPOLOGY.ja.md`](v2/V2_HOSTED_HANDOFF_TOPOLOGY.ja.md) で定義します。
