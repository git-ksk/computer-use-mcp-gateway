...
| Recovery dogfood hardening | #253, #254, #115, #255 | #253/#254/#115 完了、#255 はこの変更で implementation complete | implementation/evidence が released safety/recovery invariant failure を示さない限り non-blocking |
...
4. **physical-acceptance 待ち時間を bounded recovery hardening に使用:** #253 installed runtime/operator-tool skew detection と #254 recovery-key readiness は完了済みで、#115 actionable `device_indeterminate` guidance は完了済みで、#255 Human historical-resolution guidance もこの変更で implementation complete です。これらは compatible な `0.4`-era dogfood improvement であり自動的な release blocker にはしません。既存 safety/recovery invariant が満たされていない evidence が出た場合だけ blocker へ昇格します。
