...
Issue [#152](https://github.com/git-ksk/computer-use-mcp-gateway/issues/152) tracks this integration. The target topology is Agent-owned: the controlled Agent owns the canonical Handoff FSM/checkpoint, WebRTC/TURN, capture, Human input, and local verification; the Hub retains CUMG authorization/ledger/quarantine and only a conservative pre-dispatch fence plus signed operator-control relay. Hub and Agent therefore do not run duplicate Handoff state machines. Live generation rollover uses an explicit same-surface `rebind_live`, and the final Agent gate re-validates the signed authority binding against the actual command immediately before Cua. The legacy Unix bridge stays compatibility/regression-only. #152 remains open until merged-main physical OS-window regression proves the full Human `Done -> verifying -> fresh stable_samples=2/no-screenshot Agent-local verification -> explicit resume` lifecycle with no auto-replay and no residual quarantine. That gate must pass before `mcp-execution-handoff#48` Terminal/PTY dogfood.
