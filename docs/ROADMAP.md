...
4. **Broaden Recovery & Reconciliation in dependency order:** #103 first extends durable effectful operation identity/status; then #137 adds reviewed local-Human current-state acceptance, #136 separates permanent replay tombstones from bounded detailed retirement history, and #217/#227/#228 complete cross-platform user-presence recovery parity.
...
- **`0.4.0` — Recovery & Reconciliation:** #103 is complete and extends caller-retained durable operation identity/status to effectful Desktop/Browser work without retaining raw GUI/browser payloads or weakening no-replay. Continue with explicit local-Human current-state acceptance for reviewed low-impact GUI ambiguity (#137), then separate permanent replay tombstones from bounded detailed retirement history (#136), followed by cross-platform user-presence recovery parity (#217/#227/#228).
...
- **`0.4.0 — Recovery & Reconciliation`:** #103 is complete: effectful Desktop/Browser calls now support caller-retained durable operation identity and owner/exact-capability-scoped status-only recovery without replay or raw GUI/browser payload retention. Next is #137 reviewed local-Human current-state acceptance, then #136 permanent replay tombstone vs bounded detailed retirement history, then #217 cross-platform user-presence recovery parity split into #227 Windows Hello/WebAuthn and #228 Linux FIDO2 UV. Provider work remains preserved but follows those shared recovery semantics unless production evidence changes priority.
