...
| Recovery dogfood hardening | #253, #254, #115, #255 | #253 complete; #254 implementation complete in this change; #115/#255 remain parallel | Non-blocking unless implementation/evidence exposes a released safety or recovery invariant failure |
...
4. **Use physical-acceptance wait time for bounded recovery hardening:** #253 installed runtime/operator-tool skew detection and #254 recovery-key readiness are complete; proceed with #115 actionable `device_indeterminate` guidance, then #255 Human historical-resolution guidance. These are compatible `0.4`-era dogfood improvements, not automatic release blockers; promote one only if its evidence shows an existing safety/recovery invariant is not actually satisfied.
...
- **`0.4.0 — Recovery & Reconciliation`:** #103, #137, and #136 are complete: permanent replay tombstones are separated from bounded detailed retirement history, and effectful Desktop/Browser calls support caller-retained durable status recovery, and reviewed low-impact `Scroll`/`MovePointer` ambiguity can use local-user-presence `current_state_accepted` without synthesizing historical completion or allowing replay. Next is #217 cross-platform user-presence recovery parity split into #227 Windows Hello/WebAuthn and #228 Linux FIDO2 UV. Provider work remains preserved but follows those shared recovery semantics unless production evidence changes priority.
