...
4. **Broaden Recovery & Reconciliation in dependency order:** #103 durable effectful operation identity/status and #137 reviewed local-Human current-state acceptance are complete; next #136 separates permanent replay tombstones from bounded detailed retirement history, then #217/#227/#228 complete cross-platform user-presence recovery parity.
...
- **`0.4.0` — Recovery & Reconciliation:** #103 and #137 are complete. Effectful Desktop/Browser work has caller-retained durable operation identity/status, and reviewed `Scroll`/`MovePointer` ambiguity can be retired as `current_state_accepted` only after exact local-user-presence authorization while historical outcome remains `Indeterminate` and the old ID remains permanently non-replayable. Next separate permanent replay tombstones from bounded detailed retirement history (#136), followed by cross-platform user-presence recovery parity (#217/#227/#228).
...
- **`0.4.0 — Recovery & Reconciliation`:** #103 and #137 are complete: effectful Desktop/Browser calls support caller-retained durable status recovery, and reviewed low-impact `Scroll`/`MovePointer` ambiguity can use local-user-presence `current_state_accepted` without synthesizing historical completion or allowing replay. Next is #136 permanent replay tombstone vs bounded detailed retirement history, then #217 cross-platform user-presence recovery parity split into #227 Windows Hello/WebAuthn and #228 Linux FIDO2 UV. Provider work remains preserved but follows those shared recovery semantics unless production evidence changes priority.
