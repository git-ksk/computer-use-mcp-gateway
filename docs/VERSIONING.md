...
Current released line: **0.8.x**. `v0.8.0` is the released Backend Portability & Recovery Safety baseline.
...
Project/crate versions, wire protocol schemas, capability-advertisement schemas, and durable-state schemas serve different purposes. The released v0.7 baseline keeps the live pairing at CONTROL_SCHEMA_VERSION = 12, capability schema 8, DEVICE_REGISTRY_SNAPSHOT_SCHEMA_VERSION = 8, and HUB_AGENT_SCHEMA_VERSION = 6, matching released v0.6 at the live protocol boundary. v0.7 separately advances durable execution-safety state to schema v14 and Agent M1 persistence to schema v6. Released v0.5 used control 10 / capability 6 / registry 8 / Hub-Agent 6; the historical #106 v0.6 development pairing used control 11 / capability 7. HUB_AGENT_SCHEMA_VERSION remains 6 because the outer signed envelope is unchanged. Mixed live versions fail closed. Released v0.8 advances only the Hub durable execution-safety state to schema v15 for #377 Hub-authoritative pre-enqueue non-delivery evidence; historical v0.7 schema-v14 state remains readable, and the live control/capability/registry/Hub-Agent pairing remains 12/8/8/6.
The v0.9.0 candidate for #283 additionally introduces **Hub M1 persistence schema 7** for provider-neutral durable revision/writer-epoch fencing; this is separate from execution-safety schema v15 and does not change the live 12/8/8/6 protocol pairing. Historical Hub M1 schema 5/6 is accepted as unfenced migration input, but the first authoritative Hub writer publishes schema 7 with a positive fence revision/epoch before serving. Schema 7 without that fence fails closed. Offline maintenance preserves legacy 5/6 writer shape, while authority-bearing maintenance on schema 7 advances revision under the existing epoch using conditional publication; a restarted Hub then acquires a strictly newer epoch.
...
The `v0.8.0` GitHub Release is **source-only**, matching the pre-1.0 publication boundary. Verified CI archives remain release evidence and are not official binary Release assets; they are never silently promoted into a supported distribution.
...
Reviewed packaged V2 CLIs also expose bounded human-readable `--version` identity containing the Cargo package version and the source commit embedded by the reviewed build path. Release-candidate smoke runs `--version` from the freshly extracted archive and requires both values to match the closed artifact manifest. This output is operator convenience only: manifest/hash verification remains authoritative for install, pairing, upgrade, and rollback. An ad-hoc build without an explicitly supplied reviewed source commit reports `commit unknown` rather than inspecting Git at runtime or fabricating identity.
