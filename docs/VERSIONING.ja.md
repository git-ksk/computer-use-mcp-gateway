...
v0.9.0 candidateの#283では、provider-neutral durable revision / writer-epoch fencingのため **Hub M1 persistence schema 7** も追加します。これはexecution-safety schema v15とは別のboundaryで、live 12/8/8/6 protocol pairingは変更しません。historical Hub M1 schema 5/6はunfenced migration inputとして受理しますが、最初のauthoritative Hub writerはservice開始前にpositive revision/epoch fence付きschema 7をpublishします。schema 7でfence欠落はfail closedです。offline maintenanceはlegacy 5/6 writer shapeを維持し、schema 7へのauthority-bearing maintenanceはexisting epochのままconditional publicationでrevisionを進めます。その後restartしたHubはstrictly newer epochを取得します。
...
- historical v0.2.x checkpoint は当時の control schema number を persisted registry / grant-ledger tag に流用していました。runtime restore が support する reviewed lineage は registry 2/capability 2、3/capability 3、4..=6/capability 4、7/capability 5、released v0.5 の 8/capability 6、historical #106 v0.6 の 8/capability 7、current v0.6 の 8/capability 8 です。grant-ledger tag は独立に versioning します。prototype tag 1、unknown/future tag、不可能な control/capability pairing は fail closed;
...
reviewed packaged V2 CLI は、Cargo package version と reviewed build path が埋め込んだ source commit を含む bounded human-readable `--version` identity も公開します。release-candidate smoke は fresh extraction 後の archive 内 binary に `--version` を実行し、両方が closed artifact manifest と一致することを要求します。この出力は operator convenience に限定し、install / pairing / upgrade / rollback の authoritative identity は引き続き manifest/hash verification です。明示的な reviewed source commit を与えない ad-hoc build は runtime で Git を読んだり identity を捏造したりせず `commit unknown` と表示します。
