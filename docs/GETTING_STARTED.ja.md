...
必要な Hub/grant/device-public/TLS/state path と Agent-facing gRPC bind を設定します。northbound MCP を公開するには、[`DEPLOYMENT.md`](DEPLOYMENT.md) に従い、loopback `CUMG_V2_MCP_BIND`、canonical public HTTPS resource、exact principal -> device -> `DeviceCapability` policy を設定します。authentication adapter は1つだけ選びます。OAuth RFC 7662 introspection、multi-principal deployment 向け signed OIDC/JWT、または明示的に single-principal とする authenticated tunnel 向け packaged trusted-proxy fixed-principal mode です。OIDC/JWT mode では [`DEPLOYMENT.md`](DEPLOYMENT.md) に従い configured issuer、exact audience、pinned HTTPS JWKS URI、asymmetric algorithm allowlist、required scopes を設定します。全adapterは CUMG authorization より前に identity を同じ `AuthenticatedClientPrincipal` へ縮約し、各modeは相互排他です。

northbound MCP は raw Cua proxy ではありません。現在の typed V2 contract は、Agent-native process/shell execution + bounded filesystem observation、same-context native element targeting/action を含む Desktop semantic capability、そして exact policy と live `CapabilityAdvertisement` が許可する場合の Browser semantic capability + bounded upload/download transfer という構成です。discovery は exact authorization と Agent の live advertisement で filter され、Agent が offline の場合は semantic device tool を公開しません。 reviewed packaged profileでは、authorized clientはHub-local/read-onlyの `cumg_status` を first diagnostic として使います。このtoolはlive Agent advertisementがなくてもdiscoverableで、`v2_status`と同じprivacy-bounded unified status schemaを返します。詳細は [`V2_GUI_SEMANTIC_CAPABILITIES.ja.md`](v2/V2_GUI_SEMANTIC_CAPABILITIES.ja.md)、[`V2_BROWSER_SEMANTIC_CAPABILITIES.md`](v2/V2_BROWSER_SEMANTIC_CAPABILITIES.md)、[`V2_CUA_PARITY_MATRIX.ja.md`](v2/V2_CUA_PARITY_MATRIX.ja.md) を参照してください。
